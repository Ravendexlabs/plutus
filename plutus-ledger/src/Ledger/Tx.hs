...
getCardanoTxId (Left (SomeTx (C.Tx body _) _)) = TxId
                                        $ PlutusTx.toBuiltin
                                        $ C.serialiseToRawBytes
                                        $ C.getTxId body
getCardanoTxId (Right tx) = txId tx

-- TODO Refactor similar code with Plutus.Contract.CardanoAPI
...
getCardanoTxInputs (Left (SomeTx (C.Tx (C.TxBody C.TxBodyContent {..}) _) _)) =
  ...
  -- Set.fromList $ fmap ((`TxIn` Nothing) . (\(C.TxIn txid (C.TxIx txIx)) -> TxOutRef (TxId $ PlutusTx.toBuiltin $ C.serialiseToRawBytes txid) (toInteger txIx)) . fst) txIns
